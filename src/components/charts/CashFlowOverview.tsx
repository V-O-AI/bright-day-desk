...
const waterfallRaw = [
  ...
  { name: "Ожидаемые поступления", value: 16200, type: "in" },
...
const topInflows = [
  ...
  { id: 2, source: "Amazon", amount: 19200, pct: 12 },
  { id: 3, source: "Ожидаемые поступления", amount: 16200, pct: 10 },
  { id: 4, source: "Продукт А", amount: 7800, pct: 5 },
];

const topOutflows = [
  { id: 1, category: "Google Ads", amount: 16100, pct: 12 },
  { id: 2, category: "Возвраты", amount: 14200, pct: 10 },
  { id: 3, category: "Логистика", amount: 11800, pct: 8 },
  { id: 4, category: "Facebook Ads", amount: 11300, pct: 8 },
];

const ordersData = [
  { orderId: "00021", sku: "91", inflow: 480, outflow: 620, net: -140 },
  { orderId: "00018", sku: "57", inflow: 920, outflow: 630, net: 290 },
  { orderId: "SKU-184", sku: "640", inflow: 640, outflow: 400, net: 240 },
];

const drawerBreakdown = [
  { sku: "SKU-91", revenue: 4800, cost: 2200, cac: 800, logistics: 600, returns: 400, net: 800 },
  { sku: "SKU-57", revenue: 9200, cost: 3800, cac: 1200, logistics: 900, returns: 200, net: 3100 },
  { sku: "SKU-184", revenue: 6400, cost: 2800, cac: 600, logistics: 500, returns: 100, net: 2400 },
];

const fmt = (v: number) => v.toLocaleString("pl-PL") + " zł";
    ...
    <div className="bg-card rounded-2xl border border-border p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard
          ...
          value={fmt(84200)}
          onClick={() => openDrawer("Current Cash Balance")}
        />
        <KpiCard
          ...
          onClick={() => openDrawer("Net Cash Flow")}
        />
        <KpiCard
          ...
          sub={
            <div className="text-xs text-muted-foreground mt-1 space-y-0.5">
              <div>Dec 10: 48 600 zł</div>
              <div>+30 days: 62 400 zł &nbsp; +60: 680 zł</div>
            </div>
          }
          onClick={() => openDrawer("Projected Cash Balance")}
        />
        <CashRunwayCard onClick={() => openDrawer("Cash Runway")} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-muted/30 rounded-xl border border-border p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <h4 className="text-sm font-semibold text-foreground">Projected Cash Flow</h4>
              <TooltipProvider>
                <UITooltip>
            ...
            <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-500" />Inflows</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-destructive" />Outflows</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-foreground" />Net Balance</span>
            </div>
          </div>
        ...
        <div className="bg-muted/30 rounded-xl border border-border p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <h4 className="text-sm font-semibold text-foreground">Cash Flow Breakdown</h4>
              <TooltipProvider>
                <UITooltip>
                  <TooltipTrigger><Info className="w-3.5 h-3.5 text-muted-foreground" /></TooltipTrigger>
          ...
          <div className="text-xs text-muted-foreground mb-1">Starting Cash: <span className="font-semibold text-foreground">84 200 zł</span></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-muted/30 rounded-xl border border-border p-4">
          <h4 className="text-sm font-semibold text-foreground mb-3">Top Inflows</h4>
          <table className="w-full text-xs">
            <thead><tr className="text-muted-foreground border-b border-border">
              <th className="text-left py-1.5 font-medium">#</th>
              <th className="text-left py-1.5 font-medium">Source</th>
              <th className="text-right py-1.5 font-medium">Inflow</th>
              <th className="text-right py-1.5 font-medium">% Tot</th>
            </tr></thead>
            <tbody>
              {topInflows.map((r) => (
                <tr key={r.id} className="border-b border-border/50 hover:bg-muted/40 cursor-pointer" onClick={() => openDrawer(`Inflow: ${r.source}`)}>
        ...
        {/* Top Outflows */}
        <div className="bg-muted/30 rounded-xl border border-border p-4">
          <h4 className="text-sm font-semibold text-foreground mb-3">Top Outflows</h4>
          <table className="w-full text-xs">
            <thead><tr className="text-muted-foreground border-b border-border">
              <th className="text-left py-1.5 font-medium">#</th>
              <th className="text-left py-1.5 font-medium">Category</th>
              <th className="text-right py-1.5 font-medium">Outflow</th>
              <th className="text-right py-1.5 font-medium">% of total</th>
            </tr></thead>
            <tbody>
              {topOutflows.map((r) => (
                <tr key={r.id} className="border-b border-border/50 hover:bg-muted/40 cursor-pointer" onClick={() => openDrawer(`Outflow: ${r.category}`)}>
                  <td className="py-2 text-muted-foreground">{r.id}</td>
        ...
        <div className="bg-muted/30 rounded-xl border border-border p-4">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-sm font-semibold text-foreground">Orders / Categories</h4>
          ...
          <table className="w-full text-xs">
            <thead><tr className="text-muted-foreground border-b border-border">
              ...
              <th className="text-right py-1.5 font-medium">Inflow</th>
              <th className="text-right py-1.5 font-medium">Net Flow</th>
            </tr></thead>
            <tbody>
              {ordersData.map((r) => (
                <tr key={r.orderId} className="border-b border-border/50 hover:bg-muted/40 cursor-pointer" onClick={() => openDrawer(`Order ${r.orderId}`)}>
      ...
      <div className="flex items-center gap-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl px-5 py-3">
        ...
        <p className="text-sm text-foreground">
          <span className="font-semibold">AI Insight:</span> If current trends continue, cash reserves will drop below{" "}
          <span className="text-destructive font-semibold italic">critical threshold</span> in <span className="font-bold">28 days</span>.
        </p>
      </div>
      ...
      <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
          ...
          <div className="mt-4 space-y-5">
            <div className="text-xs text-muted-foreground">Период: Last {dateRange} days</div>
            {/* Breakdown table */}
            <div>
              <h4 className="text-sm font-semibold mb-2">Детализация по SKU</h4>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                {[
                  ...
                  { segment: "Регион A", net: 8600 },
                  { segment: "Регион B", net: 4200 },
                ].map((s) => (
...
function CashRunwayCard({ onClick }: { onClick: () => void }) {
  ...
  return (
    <div className="bg-muted/30 border border-border rounded-xl p-4 cursor-pointer hover:shadow-md transition-shadow" onClick={onClick}>
      ...
      <div className="text-2xl font-bold text-amber-600">{days} days</div>
