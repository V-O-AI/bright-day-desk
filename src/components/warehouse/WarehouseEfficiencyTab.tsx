...
import { TrendingUp, TrendingDown, Info, ArrowUp, ArrowDown, ArrowUpDown } from "lucide-react";
...
function getAgingData(abc: "A" | "B" | "C", level: AgingLevel) {
  ...
  const ranges: Record<AgingLevel, [number, number]> = { normal: [0, 3], slowing: [4, 7], risk: [8, 14], problem: [15, 30], critical: [31, 999] };
...
const bubbleData = products.map(p => ({
  x: p.stockoutRisk, y: p.profitability, z: p.remaining * p.price / 10000,
  name: p.name, ...p,
  segment: p.profitability >= 50 && p.stockoutRisk < 50 ? "stable" : p.profitability >= 50 && p.stockoutRisk >= 50 ? "maintain" : p.profitability < 50 && p.stockoutRisk < 50 ? "optimize" : "action",
}));

const segmentColors: Record<string, string> = { stable: "hsl(142, 71%, 45%)", maintain: "hsl(142, 50%, 55%)", optimize: "hsl(35, 92%, 50%)", action: "hsl(0, 72%, 55%)" };
const segmentLabels: Record<string, string> = { stable: "Стабильные", optimize: "Оптимизировать", maintain: "Поддержать", action: "Принять меры" };
...
function BubbleTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-popover border border-border rounded-lg p-3 shadow-xl text-xs max-w-[240px]">
      ...
      <p className="text-muted-foreground">Остаток: {d.remaining} шт</p>
      <p className="text-muted-foreground">В пути: {d.inTransit} шт</p>
      <p className="text-muted-foreground">Продажи/день: {d.salesPerDay}</p>
      <p className="text-muted-foreground">Тренд: {d.salesTrend > 0 ? "+" : ""}{d.salesTrend}%</p>
      <p className="text-muted-foreground">Выкуп: {d.buyoutRate}%</p>
      <p className="text-muted-foreground">Закончится: {d.daysLeft} дн.</p>
      <p className="mt-2 text-primary font-medium">Клик → детали</p>
    </div>
  );
}
...
export function WarehouseEfficiencyTab() {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
        ...
        <div className="h-[280px]">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={salesVsStockMonthly}>
              ...
              <YAxis yAxisId="left" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} tickFormatter={v => `${(v / 1000).toFixed(0)}k`} />
              <YAxis yAxisId="right" orientation="right" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} tickFormatter={v => `${(v / 1000000).toFixed(0)}M`} />
              <RechartsTooltip content={<SalesStockTooltip />} />
              <ReferenceLine yAxisId="right" y={2000000} stroke="hsl(var(--muted-foreground))" strokeDasharray="5 5" />
      ...
      <AiInsightBlock text="Июль показал пик продаж (520к шт) при запасах 5.2М — оптимальное соотношение. В декабре запасы снизились до 3.5М, что может привести к дефициту в январе." blockTitle="Продажи vs Запасы" />
                  ...
                  {(["A", "B", "C"] as const).map(abc => {
                    ...
                    return (
                      <td key={abc} className="p-1">
                        <div onClick={() => data.count > 0 && setAgingModal({ abc, level: row.label, items: data.items })}
      ...
      <AiInsightBlock text="В категории C — 2 товара в критической зоне (31+ дней без продаж). Рекомендуется провести распродажу Свечей зажигания и рассмотреть вывод Чайника электрического из ассортимента." blockTitle="ABC Aging Matrix" />
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <div className="h-[350px] relative">
          <div className="absolute top-2 left-12 text-xs font-semibold text-green-600">★ Стабильные</div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
              <XAxis type="number" dataKey="x" domain={[0, 100]} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} />
              <YAxis type="number" dataKey="y" domain={[0, 100]} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} />
      ...
      <AiInsightBlock text={`${Math.round(bubbleData.filter(b => b.segment === "action").length / bubbleData.length * 100)}% товаров в зоне «Принять меры». Снижение риска дефицита для этих товаров увеличит выручку на ~115 000 ₽ в месяц.`} blockTitle="Матрица товаров" />

      {/* Aging Modal */}
      <Dialog open={!!agingModal} onOpenChange={() => setAgingModal(null)}>
        <DialogContent className="max-w-md max-h-[70vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Категория {agingModal?.abc} — {agingModal?.level}</DialogTitle></DialogHeader>
          <div className="space-y-2">
            ...
            {agingModal?.items.length === 0 && <p className="text-muted-foreground text-sm text-center py-4">Нет товаров</p>}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!bubbleModal} onOpenChange={() => setBubbleModal(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{bubbleModal?.name}</DialogTitle></DialogHeader>
          {bubbleModal && (
            <div className="space-y-3 text-sm">
              ...
              <div className="p-3 bg-primary/5 rounded-lg border border-primary/10">
                <p className="text-xs text-muted-foreground"><Info className="h-3.5 w-3.5 inline mr-1 text-primary" />Рекомендация: {bubbleModal.riskScore >= 70 ? "Срочно пополнить запасы." : bubbleModal.riskScore >= 40 ? "Мониторить уровень запасов." : "Запасов достаточно."}</p>
              </div>
            </div>
          )}
        </DialogContent>
